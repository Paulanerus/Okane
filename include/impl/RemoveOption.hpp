...
public:
    ...
    void execute(const std::vector<std::string>& args) override
    {
        ...
        if (index < month_entry->get().entries.size() && month_entry->get().entries[index].type == okane::entry::EntryType::Abo) {
            ...
            auto abo_entry = std::find_if(abos.begin(), abos.end(), [&](const okane::entry::Entry& abo) { return abo.tag == month_entry->get().entries[index].tag && abo.amount == month_entry->get().entries[index].amount; });
